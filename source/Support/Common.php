...
class Common
{
    ...
    public static function filterRecursiveSensitiveValue(array $array): array
    {
        $sensitiveKeys = explode(',', Env::get('FILTER_SENSITIVE_VALUES', ''));

        foreach ($array as $key => $value) {
            if (is_array($value)) {
                $array[$key] = self::filterRecursiveSensitiveValue($value);
            }
